...
fn max_seperation(centroids: &DMatrix<f32>) -> (usize, usize, Vec<f32>, Vec<f32>){
    ...
    let centroid_is_zero: bool = centroid.iter().all(|&x| x == 0.0);
    ...
    let centroid_has_non_zero = centroid.iter().any(|&x| x != 0.0);
    ...
    let mut query_norm_sq = centroid.dot(&centroid);
    ...
    for i in 0..a_centroid.len() {
        ...
        sims_med.push(round_f32(score, 4));
    }
    ...
    debug!("mol1: {}, a_mol1: = {:?}, centroids * centroids.row(mol1).transpose(): = {}", 
    mol1,
    a_mol1, 
    ...
    for i in 0..a_mol1.len() {
        ...
        debug_assert!(score >= 0.0 && score <= 1.0 );
        sims_mol1.push(round_f32(score, 4));
    }
    ...
    for i in 0..a_mol2.len() {
        ...
        debug_assert!(score >= 0.0 && score <= 1.0 );
        sims_mol2.push(round_f32(score, 4));
    }
...
fn calc_centroid( ls: &Vec<f32>, nj: u32, n_features: usize) -> RowVector::<f32, Dyn, VecStorage<f32, U1, Dyn>> {
...
fn split_node(
    ...
    let (
        ...
        node1_dist,
        node2_dist
    ) = max_seperation(&node.centroids.as_ref().unwrap());
...
fn find_closest_subluster(
    ...
    centroid: &RowVector::<f32, Dyn, VecStorage<f32, U1, Dyn>>
) -> usize {
    ...
    let query_norm_sq = centroid.dot(&centroid);
    ...
    pub fn insert_bf_subcluster(
        ...
        if closest_node_is_none {
            let merged = self.subclusters.as_mut().unwrap()[row_idx].merge_subcluster(
                &subcluster, threshold, write_out
            );
...
impl BFSubcluster {
    pub fn new(
        ...
        if linear_sum.as_ref() == None {
            ...
            BFSubcluster {
                ...
                child: None,
                centroid: Some(
                    RowVector::<f32, Dyn, VecStorage<f32, U1, Dyn>>::zeros(
                        n_features
                    )
                ),
            }
            ...
            let centroid_from_vec = RowVector::<f32, Dyn, VecStorage<f32, U1, Dyn>>::from_vec(
            ...
            BFSubcluster {
                ...
                child: None,
                centroid: Some(centroid_from_vec),
            }
    ...
    pub fn update(& mut self, subcluster: &BFSubcluster, n_features: usize) {
        ...
        // with any other data type. We need max_branches and n_featires 
        // to retern a generated Dmatrix for self.centroid. 
        self.centroid = Some(calc_centroid(
    ...
    fn merge_subcluster (
        ...
        let new_centroid = calc_centroid(&new_ls, new_n, new_ls.len() );
        ...
        let row_as_vec: Vec<f32> = new_centroid.iter().cloned().collect();
